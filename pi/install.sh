...
if [[ "$RECONFIGURE" == "true" ]]; then
    ...
    echo
    # Write config — restrict permissions first, then write, so the token is
...
NTMS_MONITOR_TOKEN=${MONITOR_TOKEN}
NTMS_BEACON_ID=${BEACON_ID}
NTMS_API_URL=${API_URL}
NTMS_PHASE_FILTER=${PHASE_FILTER}
